...
describe('middleware', function () {
  ...
  it('calls next() if the test pass', function (done) {
    p.getPermissions.returns({0: true});
    var mid = middleware(p);
    mid(req, res, done);
  });

  it('does not call next() if the test fails', function (done) {
  ...
  it('sends HTTP status 403 (FORBIDDEN) if test fails', function (done) {
    p.getPermissions.returns({0: false});
    var mid = middleware(p);
    mid(req, res, next);
  ...
  describe('.create()', function () {
    it('aliases Permission.create()', function () {
      expect(middleware.create).to.equal(Permission.create);
    });
  });
});